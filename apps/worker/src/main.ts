...
const apiBaseUrl = process.env.API_INTERNAL_BASE_URL || 'http://localhost:3000/api';
...
function sendJson(res: import('node:http').ServerResponse, statusCode: number, payload: object) {
...
async function checkApi(): Promise<DependencyStatus> {
	try {
		...
		return payload.status === 'ok' || payload.status === 'degraded' ? 'up' : 'down';
	} catch {
		return 'down';
	}
}
...
async function bootstrap() {
	...
	if (startupHealth.dependencies.redis === 'down' || startupHealth.dependencies.api === 'down') {
	...
	server.listen(workerPort, () => {
		console.log(`Worker running with health endpoint on http://localhost:${workerPort}/health`);
	});
